...
# HABITAT -----
metadata <- read_metadata(here::here("data/2019-08-Ningaloo-deep_stereo-BRUVS/")) %>%
...
forwards <- read.delim("data/2019-08-Ningaloo-deep_stereo-BRUVS/2019-08_ningaloo_forwards_Dot Point Measurements.txt", 
                       header = T, skip = 4, stringsAsFactors = FALSE, 
                       colClasses = "character", na.strings = "") %>%
  clean_names()

# read in forwards annotations
backwards <- read.delim("data/2019-08-Ningaloo-deep_stereo-BRUVS/2019-08_ningaloo_backwards_Dot Point Measurements.txt", 
                       header = T, skip = 4, stringsAsFactors = FALSE, 
                       colClasses = "character", na.strings = "") %>%
  clean_names()
...
tidy_habitat <- habitat_with_schema %>%
  ...
  dplyr::mutate(campaignid = "2019-08-Ningaloo-deep_stereo-BRUVS") %>%
...
write_csv(tidy_habitat, "data/to upload/2019-08-Ningaloo-deep_stereo-BRUVS_benthos.csv")


# RELIEF ----
relief_with_schema <- bind_rows(forwards, backwards) %>%
  dplyr::filter(!is.na(relief)) %>%
  dplyr::mutate(level_5 = str_sub(relief, 1, 1)) %>%
  dplyr::left_join(catami) %>%
  dplyr::mutate(sample = str_replace_all(filename, c(".JPG"= "", ".jpg" = ""))) 

unique(relief_with_schema$relief)
...
write_csv(tidy_relief, "data/to upload/2019-08-Ningaloo-deep_stereo-BRUVS_relief.csv")
